...
public class CRUD {
	...
	public void updateById(int idx, String nome, String origem, int unidade, double preco, String fabricante) {
		for (int i = 0; i < id; i++) {
			if (cc[i].getId() == idx) {
				cc[i].setNome(nome);
				cc[i].setOrigem(origem);
				cc[i].setUnidade(unidade);
				cc[i].setPreco(preco);
